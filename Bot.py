...
def get_query_embedding(query):
    ...
    return 1#torch.tensor(sentence_model.encode(query))


def get_movie_embeddings(descriptions):
    """Отримання векторного подання описів фільмів за допомогою Sentence-BERT"""
    return 1#[torch.tensor(sentence_model.encode(desc)) for desc in tqdm(descriptions, desc="Обчислення ембеддінгів для описів фільмів")]
...
def get_recommendations_with_hypotheses(query, movies, content_type=None, user_history=None, like_count=None,
                                        trending_now=None, top_n=5):
...
def get_recommendations_by_user_input(selected_genres, selected_tags, selected_type):
    if selected_type == "book":
        ...
        filtered_books = filtered_books[
            filtered_books['genres'].apply(lambda x: any(genre in x for genre in selected_genres))]
    ...
    if selected_tags:
        filtered_movies = filtered_movies[
            filtered_movies['genres'].apply(lambda x: any(genre in x for genre in selected_genres)) &
...
def load_user_history(file_path):
    try:
        ...
        for user_id, feedback in user_feedback.items():
            ...
            user_history.extend(
                [{'user_id': user_id, 'movie': movie, 'feedback': 'disliked'} for movie in disliked_movies])
...
def perform_description_search(message):
    ...
    # Перевірка, чи є тип
    if 'type' not in user_data.get(chat_id, {}):
        print(f"Помилка: відсутній тип для користувача {chat_id}")
        bot.send_message(chat_id, "Помилка: не вибрано категорію. Спочатку виберіть категорію.")
        return
    ...
    if 'search_mode' in user_data[chat_id] and user_data[chat_id]['search_mode'] == 'description':
        ...
        result = get_recommendations_with_hypotheses(search_query, movies, content_type, user_history, like_count,
                                                     trending_now=None, top_n=5)
        print(result)
    ...
    if index < len(recommendations):
        ...
        if selected_type == 'book':
            ...
            bot.send_message(
                ...
                "\n".join(
                    [f"{book['title']} by {book['author']} - {book['url']}" for book in row['readers_also_enjoyed']])
            )
        else:
